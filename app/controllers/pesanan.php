        ...
        if($pesanan != null){
            if($this->model('PesananModel')->cek_pesanan(1) == null){
                ...
                $pesan = [
                    ...
                    'keterangan' => 'Pesanan Anda kepada Arsitek dengan Produk <a href="'.BASEURL.'/produk/'.$pesanan['id_produk'].'">'.$pesanan->judul.'</a> telah diterima, harap masukkan IMB Anda secepatnya.',
                    'link' => '/pengguna/imb_pesanan/'.$pesanan['id_pesanan']
                ];
    ...
    public function tolak($id_pesanan)
    {
        ...
        if($pesanan != null){
            ...
            $pesan = [
                ...
                'keterangan' => 'Pesanan Anda kepada Arsitek dengan Produk <a href="'.BASEURL.'/produk/'.$pesanan['id_produk'].'">'.$pesanan->judul.'</a> telah ditolak, klik untuk temukan arsitek lain.',
                'link' => 'index'
            ];
            $this->model('NotifikasiModel')->notifikasi($pesan);
            $this->route('arsitek/pesanan');
